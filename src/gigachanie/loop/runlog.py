...
def git_changed_files(root: Path) -> list[str]:
    ...
    `.agent/`(체크포인트·세션·로그 등 GigaChanie 자체 상태)는 사용자가
    아직 gitignore 하지 않았더라도 "코드 변경"이 아니므로 항상 제외한다.
    """
    ...
    for f in (*tracked, *untracked):
        if f in seen or f.startswith(_AGENT_DIR_PREFIX):
            continue
        seen.add(f)
        out.append(f)
    return out
