...
- VS Code 확장에서 에이전트가 `ask_user` 로 질문하면 화면 상단에 팝업(QuickPick/입력창)이 떠 채팅 흐름을 끊고, 취소 시 빈 답이 되어 약한 모델이 같은 질문을 계속 되물어 작업이 진행되지 않던 문제 수정. 질문을 **채팅 안 인라인 카드**(옵션 버튼)로 표시하고, 자유 답변은 **기존 하단 입력창을 그대로 재사용**(질문 대기 중엔 입력창 테두리가 강조되고 다음 입력이 답으로 전송됨) — 카드 안에 별도 입력칸을 만들지 않는다. `ask_user` 를 3회 연속 호출하면 그 뒤로는 차단해 "지금 정보로 진행" 을 강제하고, `ask_user` 결과 메시지도 "이 답변대로 진행, 다시 묻지 말 것" 으로 더 단호하게 (#65)
- `giga ensemble "질문" -m A -m B` 처럼 **판정 모델(`-j`)을 생략**하면 첫 번째 멤버 모델을 판정에도 그대로 재사용했는데, 그 멤버는 이미 답변 후 `release()` 로 백엔드(연결)를 닫은 상태라 판정 호출이 닫힌 연결에 나가 실제 백엔드(Ollama 등)에서 처리되지 않은 예외로 크래시하던 문제 수정 — 판정용 백엔드는 항상 새 인스턴스로 만든다 (#66)
- 에이전트가 **새로 만든(아직 커밋 안 된) 파일**이 `git diff --name-only HEAD` 기준으로만 잡히는 "변경 파일" 목록·검토 diff 에서 통째로 빠지던 문제 수정. `giga agent --json` 의 `changed_files`, `giga serve` 가 VS Code 확장에 보내는 `changedFiles`(변경 파일 클릭-열기 목록), `giga agent --review`/`--review-fix` 가 검토 모델에 보내는 diff 모두 영향받았음 — 이제 `git ls-files --others --exclude-standard` 로 찾은 미추적 새 파일도 목록·diff(새 파일 전체를 "새 파일" diff 로 합성)에 포함한다 (#67)
- `giga chat`/`giga serve` 로 **커밋 없이 여러 턴을 거치면** `changedFiles`(확장의 변경 파일 목록)·실행 로그의 `changed_files` 가 매 턴 `git diff HEAD` 를 다시 돌려 **이전 턴에서 이미 보여준 파일까지 계속 다시 끼어** 나오던 문제 수정 — "지금 이 턴이 뭘 바꿨는지"가 아니라 "HEAD 대비 지금 전체 상태"였기 때문. 체크포인트(`--no-checkpoint` 로 안 끈 경우)가 턴 단위로 정확히 기록해 둔 파일 목록을 대신 쓴다. `giga agent` 단발 실행의 `--json` 출력은 그대로 `git diff` 기준(한 커맨드 안에서 `--review-fix` 가 만드는 두 번째 턴까지 합쳐 보여주는 게 맞아서) (#68)
- `#67` 의 untracked-파일 포함 수정이 부작용으로 `.agent/`(체크포인트 스냅샷·실행 로그 등 GigaChanie 자체 상태)까지 "변경 파일"에 끼워 넣던 문제 수정 — 실기동으로 `giga agent` 를 돌려 보다가 `changed_files` 에 `.agent/checkpoints/blobs/...` 가 섞여 나오는 걸 발견. `giga init` 이 `.agent/` 를 자동으로 gitignore 해 주지 않으므로 사용자가 직접 안 막아 뒀으면 그대로 새던 상황 — 이제 `.agent/` 경로는 항상 제외한다 (#69)

### 추가
- `write_file`/`apply_edit`: 대상이 `.py` 파일이면 쓰기 직후 `compile()` 로 구문을 검사해, 결과가 깨졌으면(예: 약한 모델이 함수 본문을 못 지워 들여쓰기가 틀어짐) 도구 결과에 `⚠ 구문 오류` 경고를 바로 붙여 준다. 치환 자체는 요청대로 됐으니 도구는 실패(`is_error`)로 안 잡지만, 모델이 같은 턴 안에서 바로 알아채고 고칠 기회를 준다. 실기동 셰이크아웃 중 `def sub(...)` 를 `multiply` 로 바꾸다 본문 들여쓰기가 깨져도 "완료"라고 답하는 걸 보고 추가 (#70)
