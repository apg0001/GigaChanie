...
- VS Code 확장에서 에이전트가 `ask_user` 로 질문하면 화면 상단에 팝업(QuickPick/입력창)이 떠 채팅 흐름을 끊고, 취소 시 빈 답이 되어 약한 모델이 같은 질문을 계속 되물어 작업이 진행되지 않던 문제 수정. 질문을 **채팅 안 인라인 카드**(옵션 버튼 + 자유입력)로 표시하고, `ask_user` 를 3회 연속 호출하면 그 뒤로는 차단해 "지금 정보로 진행" 을 강제한다. `ask_user` 결과 메시지도 "이 답변대로 진행, 다시 묻지 말 것" 으로 더 단호하게 (#65)
